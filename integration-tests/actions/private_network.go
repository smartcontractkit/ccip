...
import (
	...
	ctf_config_type "github.com/smartcontractkit/chainlink-testing-framework/config/types"
...
func EthereumNetworkConfigFromConfig(l zerolog.Logger, config ctf_config.GlobalTestConfig) (network ctf_test_env.EthereumNetwork, err error) {
	if config.GetPrivateEthereumNetworkConfig() == nil {
		...
		network, err = ethBuilder.
			WithEthereumVersion(ctf_config_type.EthereumVersion_Eth1).
			WithExecutionLayer(ctf_config_type.ExecutionLayer_Geth).
			Build()
