...
func SetNodeConfig(evmNetworks []blockchain.EVMNetwork, nodeConfig, commonChain string, configByChain map[string]string) (*corechainlink.Config, string, error) {
	...
	if nodeConfig == "" {
		tomlCfg = integrationnodes.NewConfig(
			...
			node.WithPrivateEVMs(evmNetworks, commonChainConfig, configByChainMap))
	} else {
		tomlCfg, err = node.NewConfigFromToml([]byte(nodeConfig), node.WithPrivateEVMs(evmNetworks, commonChainConfig, configByChainMap))
		if err != nil {
			return nil, "", err
		}
	}
