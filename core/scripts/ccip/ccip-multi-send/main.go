...
func MultiCallCCIP(
	...
	boundContract := bind.NewBoundContract(contractAddress, multiCallABI, ethC, ethC, ethC)
	var out []interface{}
	...
	inputs, err := ccipSend.Inputs.Pack(
		msgData.ChainSelector,
		msgData.MSG,
	)
	inputs = append(sendID[:], inputs[:]...)

	data := Call{target: msgData.RouterAddr, callData: inputs}
	err := boundContract.Call(nil, &out, "tryAggregate", data)
}
