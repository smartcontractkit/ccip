...
func TestDecodeRLP(t *testing.T) {
	type testCase struct {
		name     string
		inputHex string
		expected func() *opstack.RawRLPOutput
	}
	tests := []testCase{
		{
			name:     "full node",
			...
			expected: func() *opstack.RawRLPOutput {
				root := opstack.NewRLPBuffers()
				root.Children = []*opstack.RawRLPOutput{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			bufs := opstack.NewRLPBuffers()
			err := rlp.DecodeBytes(decoded, bufs)
			require.NoError(t, err)
			require.Equal(t, test.expected(), bufs)
		})
	}
}
