...
func setOffRampOnTokenPools(t *testing.T, destClient *EvmChainConfig) {
	for _, tokenPoolAddress := range destClient.TokenPools {
		pool, err := native_token_pool.NewNativeTokenPool(tokenPoolAddress, destClient.Client)
		require.NoError(t, err)

		// Configure offramp address on pool
		tx, err := pool.SetOffRamp(destClient.Owner, destClient.OffRamp, true)
		require.NoError(t, err)
...
func setOnRampOnOnRampRouter(t *testing.T, sourceClient *EvmChainConfig, destChainId *big.Int) {
	sourceClient.Logger.Infof("Setting the onRamp on the onRampRouter")
	onRampRouter, err := evm_2_any_subscription_onramp_router.NewEVM2AnySubscriptionOnRampRouter(sourceClient.OnRampRouter, sourceClient.Client)
	require.NoError(t, err)
	sourceClient.Logger.Infof("Registering new onRamp")
	tx, err := onRampRouter.SetOnRamp(sourceClient.Owner, destChainId, sourceClient.OnRamp)
	require.NoError(t, err)
	shared.WaitForMined(t, sourceClient.Logger, sourceClient.Client, tx.Hash(), true)
}

func setOnRampOnTokenPools(t *testing.T, sourceClient *EvmChainConfig) {
	for _, tokenPoolAddress := range sourceClient.TokenPools {
		pool, err := native_token_pool.NewNativeTokenPool(tokenPoolAddress, sourceClient.Client)
		require.NoError(t, err)

		// Configure offramp address on pool
		tx, err := pool.SetOnRamp(sourceClient.Owner, sourceClient.OnRamp, true)
		require.NoError(t, err)
		shared.WaitForMined(t, sourceClient.Logger, sourceClient.Client, tx.Hash(), true)
		sourceClient.Logger.Infof("Onramp pool configured with offramp address: %s", helpers.ExplorerLink(sourceClient.ChainId.Int64(), tx.Hash()))
	}
}

func setOnRampOnBlobVerifier(t *testing.T, sourceClient *EvmChainConfig, destClient *EvmChainConfig) {
	...
	config.OnRamps = append(config.OnRamps, sourceClient.OnRamp)
	config.MinSeqNrByOnRamp = append(config.MinSeqNrByOnRamp, 1)

	tx, err := blobVerifier.SetConfig(destClient.Owner, config)
	require.NoError(t, err)
	destClient.Logger.Infof(fmt.Sprintf("Adding new onRamp to blobVerifier in tx %s", helpers.ExplorerLink(destClient.ChainId.Int64(), tx.Hash())))
}

func setOffRampRouterOnOffRamp(t *testing.T, destClient *EvmChainConfig) {
	offRamp, err := any_2_evm_free_offramp.NewEVM2EVMFreeOffRamp(destClient.OffRamp, destClient.Client)
	require.NoError(t, err)

	tx, err := offRamp.SetRouter(destClient.Owner, destClient.OffRampRouter)
	require.NoError(t, err)
	shared.WaitForMined(t, destClient.Logger, destClient.Client, tx.Hash(), true)
...
func setOffRampOnOffRampRouter(t *testing.T, destClient *EvmChainConfig) {
	...
	tx, err := offRampRouter.AddOffRamp(destClient.Owner, destClient.OffRamp)
	require.NoError(t, err)
	shared.WaitForMined(t, destClient.Logger, destClient.Client, tx.Hash(), true)
}

/*
func revokeOffRampOnOffRampRouter(t *testing.T, destClient *EvmChainConfig, offRamp common.Address) {
...
/*
func revokeOffRampOnTokenPools(t *testing.T, destClient *EvmChainConfig, offRamp common.Address) {
	// TODO
}
*/
