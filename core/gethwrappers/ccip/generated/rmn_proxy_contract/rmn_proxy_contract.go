...
func DeployRMNProxyContract(auth *bind.TransactOpts, backend bind.ContractBackend, arm common.Address) (common.Address, *CustomTransaction, *RMNProxyContract, error) {
	...
	case 324, 280, 300:
		return DeployZkSyncRMNProxyContract(auth, backend, arm)
	}
...
func DeployZkSyncRMNProxyContract(auth *bind.TransactOpts, backend bind.ContractBackend, params ...interface{}) (common.Address, *CustomTransaction, *RMNProxyContract, error) {
	client, ok := backend.(*ethclient.Client)
	if !ok {
		return common.Address{}, nil, nil, errors.New("backend is not an ethclient")
	}
	...
	decodedBytes := common.FromHex(RMNProxyContractZkBin)
	fmt.Println("deploying")
	RMNProxyContractAbi, err := RMNProxyContractMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	constructor, err := RMNProxyContractAbi.Pack("", params...)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
