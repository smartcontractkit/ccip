...
func (r CCIPReader) CommitReportsGTETimestamp(ctx context.Context, dest cciptypes.ChainSelector, ts time.Time,
...
func (r CCIPReader) ExecutedMessageRanges(ctx context.Context, source, dest cciptypes.ChainSelector,
	seqNumRange cciptypes.SeqNumRange) ([]cciptypes.SeqNumRange, error) {
	args := r.Called(ctx, source, dest, seqNumRange)
	return args.Get(0).([]cciptypes.SeqNumRange), args.Error(1)
}

func (r CCIPReader) MsgsBetweenSeqNums(ctx context.Context, chain cciptypes.ChainSelector,
	seqNumRange cciptypes.SeqNumRange) ([]cciptypes.CCIPMsg, error) {
