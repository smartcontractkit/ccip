...
func (d *Delegate) cleanupEVM(jb job.Job, q pg.Queryer, relayID relay.ID) error {
	...
	case types.CCIPCommit:
		err = ccip.UnregisterCommitPluginLpFilters(context.Background(), d.lggr, jb, d.pipelineRunner, d.legacyChains)
		if err != nil {
			d.lggr.Errorw("failed to unregister ccip commit plugin filters", "err", err, "spec", spec)
		}
		return nil
	case types.CCIPExecution:
		err = ccip.UnregisterExecPluginLpFilters(context.Background(), d.lggr, jb, d.legacyChains)
		if err != nil {
