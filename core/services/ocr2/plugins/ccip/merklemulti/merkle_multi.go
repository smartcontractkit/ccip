...
type Ctx struct {
	hashLeaf     func(l []byte) Hash
	hashInternal func(a, b Hash) Hash
	zeroHash     Hash
}
...
func NewKeccakCtx() Ctx {
	...
	return Ctx{
		hashLeaf: func(l []byte) Hash {
			h, _ := utils.Keccak256(l)
			return h
		},
		hashInternal: func(a, b Hash) Hash {
			if bytes.Compare(a, b) < 0 {
				h, _ := utils.Keccak256(append(a, b...))
				return h
			}
			h, _ := utils.Keccak256(append(b, a...))
			return h
		},
		zeroHash: []byte{0x00}, // TODO: is this acceptable?
	}
}
...
func NewTree(ctx Ctx, leaves []Hash) *Tree {
	var layer = make([]Hash, len(leaves))
	copy(layer, leaves)
...
func padLayer(ctx Ctx, layer []Hash) []Hash {
	if len(layer) == 1 {
		return layer
	}
	if len(layer)%2 != 0 {
