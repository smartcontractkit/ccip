...
func TestUSDCService_callAttestationApi(t *testing.T) {
	...
	usdcService := NewUSDCService(nil, mockOnRampAddress, "https://iris-api-sandbox.circle.com", 420)
...
func TestUSDCService_callAttestationApiMock(t *testing.T) {
	...
	usdcService := NewUSDCService(nil, mockOnRampAddress, ts.URL, 420)
...
//
//	usdcService := NewUSDCService(nil, mockOnRampAddress, ts.URL, 420)
//	isReady, attestation, err := usdcService.IsAttestationComplete(context.Background(), 245235)
//	require.NoError(t, err)
//
