...
func TestCommitInflight(t *testing.T) {
	...
	require.NoError(t, c.add(lggr, commit_store.CommitStoreReport{
	...
	require.NoError(t, c.add(lggr, commit_store.CommitStoreReport{
		Interval:   commit_store.CommitStoreInterval{Min: 3, Max: 4},
		MerkleRoot: root2,
		GasPrices: []price_registry.GasPrice{
...
func Test_inflightCommitReportsContainer_expire(t *testing.T) {
	c := &inflightCommitReportsContainer{
		...
		inFlightPriceUpdates: []InflightPriceUpdate{
			{
				gasPrices:     []price_registry.GasPrice{{DestChainSelector: 100, Value: big.NewInt(0)}},
				createdAt:     time.Now().Add(-PRICE_EXPIRY_MULTIPLIER * time.Minute),
				epochAndRound: ccipcalc.MergeEpochAndRound(10, 5),
			},
			{
				gasPrices:     []price_registry.GasPrice{{DestChainSelector: 200, Value: big.NewInt(0)}},
