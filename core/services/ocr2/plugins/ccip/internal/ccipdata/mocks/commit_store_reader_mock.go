...
// GetCommitReportsForExecution provides a mock function with given fields: ctx, logsAge, ignoredRoots
func (_m *CommitStoreReader) GetCommitReportsForExecution(ctx context.Context, logsAge time.Duration, ignoredRoots [][32]byte) ([]ccip.CommitStoreReportWithTxMeta, error) {
	ret := _m.Called(ctx, logsAge, ignoredRoots)

	if len(ret) == 0 {
		panic("no return value specified for GetCommitReportsForExecution")
	}

	var r0 []ccip.CommitStoreReportWithTxMeta
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, time.Duration, [][32]byte) ([]ccip.CommitStoreReportWithTxMeta, error)); ok {
		return rf(ctx, logsAge, ignoredRoots)
	}
	if rf, ok := ret.Get(0).(func(context.Context, time.Duration, [][32]byte) []ccip.CommitStoreReportWithTxMeta); ok {
		r0 = rf(ctx, logsAge, ignoredRoots)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]ccip.CommitStoreReportWithTxMeta)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, time.Duration, [][32]byte) error); ok {
		r1 = rf(ctx, logsAge, ignoredRoots)
	} else {
		r1 = ret.Error(1)
	}
