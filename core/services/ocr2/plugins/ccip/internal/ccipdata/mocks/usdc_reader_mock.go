...
// GetUSDCMessagePriorToLogIndexInTx provides a mock function with given fields: ctx, logIndex, offsetFromLast, txHash
func (_m *USDCReader) GetUSDCMessagePriorToLogIndexInTx(ctx context.Context, logIndex int64, offsetFromLast int, txHash string) ([]byte, error) {
	ret := _m.Called(ctx, logIndex, offsetFromLast, txHash)
	...
	if rf, ok := ret.Get(0).(func(context.Context, int64, int, string) ([]byte, error)); ok {
		return rf(ctx, logIndex, offsetFromLast, txHash)
	}
	if rf, ok := ret.Get(0).(func(context.Context, int64, int, string) []byte); ok {
		r0 = rf(ctx, logIndex, offsetFromLast, txHash)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]byte)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, int64, int, string) error); ok {
		r1 = rf(ctx, logIndex, offsetFromLast, txHash)
	} else {
		r1 = ret.Error(1)
	}
