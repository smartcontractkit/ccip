...
// Backwards compat for integration tests
func DecodeOffRampMessageV1_3_0(b []byte) (*evm_2_evm_offramp.InternalEVM2EVMMessage, error) {
	offRampABI, err := abi.JSON(strings.NewReader(evm_2_evm_offramp.EVM2EVMOffRampABI))
	if err != nil {
		panic(err)
	}
	event, ok := offRampABI.Events[EVM2EVMOffRampEventNameV1_3_0]
	if !ok {
		panic("no such event")
	}
...
func (o *OnRampV1_3_0) GetSendRequestsBetweenSeqNums(ctx context.Context, seqNumMin, seqNumMax uint64) ([]Event[internal.EVM2EVMMessage], error) {
