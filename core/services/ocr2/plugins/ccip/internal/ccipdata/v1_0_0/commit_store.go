...
func encodeCommitReport(commitReportArgs abi.Arguments, report cciptypes.CommitStoreReport) ([]byte, error) {
	...
	for _, tokenPriceUpdate := range report.TokenPrices {
		sourceTokenEvmAddr, err := tokenPriceUpdate.Token.ToEVM()
		if err != nil {
			return nil, err
		}
...
func (c *CommitStore) GetAcceptedCommitReportsGteTimestamp(ctx context.Context, ts time.Time, confs int) ([]cciptypes.CommitStoreReportWithBlockMeta, error) {
	...
	parsedReports := make([]cciptypes.CommitStoreReportWithBlockMeta, 0, len(parsedLogs))
	for _, log := range parsedLogs {
		parsedReports = append(parsedReports, cciptypes.CommitStoreReportWithBlockMeta{
			BlockMeta:         log.BlockMeta,
			CommitStoreReport: log.Data,
		})
	}
