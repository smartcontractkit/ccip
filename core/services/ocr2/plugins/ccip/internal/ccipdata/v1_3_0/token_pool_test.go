...
func TestTokenPool(t *testing.T) {
	...
	remoteChainSelector := uint64(1231232313123)
	...
	for _, tokenPoolType := range poolTypes {
		...
		pool := NewTokenPool(tokenPoolType, poolAddress, remoteChainSelector, lp, batchCallerMock)
		...
		assert.Equal(t, "getCurrentInboundRateLimiterState", call.MethodName())
		...
		pool2 := NewTokenPool(tokenPoolType, poolAddress, remoteChainSelector, lp, batchCallerMock)
