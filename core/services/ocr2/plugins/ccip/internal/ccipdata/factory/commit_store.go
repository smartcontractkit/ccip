...
func NewCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, estimator gas.EvmFeeEstimator, maxGasPrice *big.Int, pgOpts ...pg.QOpt) (ccipdata.CommitStoreReader, error) {
	return initOrCloseCommitStoreReader(lggr, versionFinder, address, ec, lp, estimator, maxGasPrice, false, pgOpts...)
}

func CloseCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, estimator gas.EvmFeeEstimator, maxGasPrice *big.Int, pgOpts ...pg.QOpt) error {
	_, err := initOrCloseCommitStoreReader(lggr, versionFinder, address, ec, lp, estimator, maxGasPrice, true, pgOpts...)
	return err
}

func initOrCloseCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, estimator gas.EvmFeeEstimator, maxGasPrice *big.Int, closeReader bool, pgOpts ...pg.QOpt) (ccipdata.CommitStoreReader, error) {
	...
	case ccipdata.V1_0_0, ccipdata.V1_1_0: // Versions are identical
		cs, err := v1_0_0.NewCommitStore(lggr, evmAddr, ec, lp, estimator, maxGasPrice)
		if err != nil {
			return nil, err
		}
		if closeReader {
