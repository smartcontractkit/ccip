...
func NewCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, dacc ccipdata.DAConfigCacheReader) (ccipdata.CommitStoreReader, error) {
	return initOrCloseCommitStoreReader(lggr, versionFinder, address, ec, lp, dacc, false)
}

func CloseCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, dacc ccipdata.DAConfigCacheReader) error {
	_, err := initOrCloseCommitStoreReader(lggr, versionFinder, address, ec, lp, dacc, true)
	return err
}

func initOrCloseCommitStoreReader(lggr logger.Logger, versionFinder VersionFinder, address cciptypes.Address, ec client.Client, lp logpoller.LogPoller, dacc ccipdata.DAConfigCacheReader, closeReader bool) (ccipdata.CommitStoreReader, error) {
	...
	case ccipdata.V1_2_0:
		cs, err := v1_2_0.NewCommitStore(lggr, evmAddr, ec, lp, dacc)
		if err != nil {
			return nil, err
		}
		if closeReader {
	...
	case ccipdata.V1_5_0:
		cs, err := v1_5_0.NewCommitStore(lggr, evmAddr, ec, lp, dacc)
		if err != nil {
			return nil, err
		}
		if closeReader {
