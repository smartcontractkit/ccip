...
func TestDynamicPriceGetterWithEmptyInput(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "aggregator_only_valid_with_input",
			param: testParamAggregatorOnlyWithInput(t),
		},
		{
		...
		{
			name:  "static_only_valid_with_input",
			param: testParamStaticOnlyWithInput(),
		},
		{
...
func testParamAggregatorOnlyWithInput(t *testing.T) testParameters {
	cfg := config.DynamicPriceGetterConfig{
		AggregatorPrices: map[common.Address]config.AggregatorPriceConfig{
			TK1: {
				ChainID:                   101,
...
func testParamStaticOnlyWithInput() testParameters {
	cfg := config.DynamicPriceGetterConfig{
		AggregatorPrices: map[common.Address]config.AggregatorPriceConfig{},
		StaticPrices: map[common.Address]config.StaticPriceConfig{
			TK1: {
