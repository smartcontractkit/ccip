	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(tc.sendReqs) > 0 {
				onRampReader.On("GetSendRequestsGteSeqNum", ctx, tc.commitStoreSeqNum, uint64(OnRampMessagesRangeScan), sourceFinalityDepth).
...
func TestCommitReportingPlugin_calculateMinMaxSequenceNumbers(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			onRampReader.On("GetSendRequestsGteSeqNum", ctx, tc.expQueryMin, uint64(OnRampMessagesRangeScan), 0).Return(sendReqs, nil)
			p.onRampReader = onRampReader
