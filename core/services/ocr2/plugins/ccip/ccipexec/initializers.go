...
func UnregisterExecPluginLpFilters(ctx context.Context, lggr logger.Logger, jb job.Job, chainSet legacyevm.LegacyChainContainer) error {
	...
	unregisterFuncs := []func() error{
		func() error {
			return factory.CloseCommitStoreReader(ctx, lggr, versionFinder, params.offRampConfig.CommitStore, params.destChain.Client(), params.destChain.LogPoller(), params.sourceChain.GasEstimator(), params.sourceChain.Config().EVM().GasEstimator().PriceMax().ToInt())
		},
		func() error {
			return factory.CloseOnRampReader(ctx, lggr, versionFinder, params.offRampConfig.SourceChainSelector, params.offRampConfig.ChainSelector, params.offRampConfig.OnRamp, params.sourceChain.LogPoller(), params.sourceChain.Client())
		},
		func() error {
			return factory.CloseOffRampReader(ctx, lggr, versionFinder, offRampAddress, params.destChain.Client(), params.destChain.LogPoller(), params.destChain.GasEstimator(), params.destChain.Config().EVM().GasEstimator().PriceMax().ToInt())
		},
		func() error { // usdc token data reader
			if usdcDisabled := params.pluginConfig.USDCConfig.AttestationAPI == ""; usdcDisabled {
				return nil
			}
			return ccipdata.CloseUSDCReader(ctx, lggr, jobIDToString(jb.ID), params.pluginConfig.USDCConfig.SourceMessageTransmitterAddress, params.sourceChain.LogPoller())
		},
	}
...
func initTokenDataProviders(ctx context.Context, lggr logger.Logger, jobID string, pluginConfig ccipconfig.ExecutionPluginJobSpecConfig, sourceLP logpoller.LogPoller) (map[cciptypes.Address]tokendata.Reader, error) {
	...
	if pluginConfig.USDCConfig.AttestationAPI != "" {
		...
		usdcReader, err := ccipdata.NewUSDCReader(ctx, lggr, jobID, pluginConfig.USDCConfig.SourceMessageTransmitterAddress, sourceLP, true)
		if err != nil {
...
func jobSpecToExecPluginConfig(ctx context.Context, lggr logger.Logger, jb job.Job, chainSet legacyevm.LegacyChainContainer) (*ExecutionPluginStaticConfig, *ccipcommon.BackfillArgs, *cache.ObservedChainHealthcheck, *tokendata.BackgroundWorker, error) {
	...
	onRampReader, err := factory.NewOnRampReader(ctx, execLggr, versionFinder, params.offRampConfig.SourceChainSelector, params.offRampConfig.ChainSelector, params.offRampConfig.OnRamp, params.sourceChain.LogPoller(), params.sourceChain.Client())
	if err != nil {
	...
	commitStoreReader, err := factory.NewCommitStoreReader(ctx, lggr, versionFinder, params.offRampConfig.CommitStore, params.destChain.Client(), params.destChain.LogPoller(), params.sourceChain.GasEstimator(), params.sourceChain.Config().EVM().GasEstimator().PriceMax().ToInt())
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "could not load commitStoreReader reader")
	}

	tokenDataProviders, err := initTokenDataProviders(ctx, lggr, jobIDToString(jb.ID), params.pluginConfig, params.sourceChain.LogPoller())
	if err != nil {
...
func extractJobSpecParams(ctx context.Context, lggr logger.Logger, jb job.Job, chainSet legacyevm.LegacyChainContainer, registerFilters bool) (*jobSpecParams, error) {
	...
	offRampReader, err := factory.NewOffRampReader(ctx, lggr, versionFinder, offRampAddress, destChain.Client(), destChain.LogPoller(), destChain.GasEstimator(), destChain.Config().EVM().GasEstimator().PriceMax().ToInt(), registerFilters)
	if err != nil {
