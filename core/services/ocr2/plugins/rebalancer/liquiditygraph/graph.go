...
type LiquidityGraph interface {
	...
	AddConnection(from, to models.NetworkSelector) (bool, error)
...
func (g *Graph) AddConnection(from, to models.NetworkSelector) (bool, error) {
	if !g.HasNetwork(from) {
		return false, fmt.Errorf("network %d not found", from)
	}
	if !g.HasNetwork(to) {
		return false, fmt.Errorf("network %d not found", to)
	}
