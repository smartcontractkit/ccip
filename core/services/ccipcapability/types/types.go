...
type OracleCreator interface {
	...
	CreateCommitOracle(config ocr3reader.OCR3ConfigWithMeta) (CCIPOracle, error)

	// CreateExecOracle creates a new oracle that will run the CCIP exec plugin.
	// The oracle must be returned unstarted.
	CreateExecOracle(config ocr3reader.OCR3ConfigWithMeta) (CCIPOracle, error)

	// CreateBootstrapOracle creates a new bootstrap node with the given OCR config.
	// The oracle must be returned unstarted.
	CreateBootstrapOracle(config ocr3reader.OCR3ConfigWithMeta) (CCIPOracle, error)
}
