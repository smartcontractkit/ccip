...
func TestIntegration_LegacyGasStation_CrossChainTransfer_SourceChain(t *testing.T) {
	...
	ccipContracts := testhelpers.SetupCCIPContracts(t, sourceChainID, testhelpers.SourceChainSelector, destChainID, testhelpers.DestChainSelector)
	...
	createLegacyGasStationServerJob(t, app, forwarderAddress, sourceChainID, testhelpers.SourceChainSelector, relayKey)
	statusUpdateServer := legacygasstation.NewUnstartedStatusUpdateServer(t)
	go statusUpdateServer.Start()
	defer statusUpdateServer.Stop()
	...
	// verify legacy_gasless_txs has correct status
	verifyTxStatus(t, orm, ccipContracts.Source.Chain, requestID, testhelpers.SourceChainSelector, types.SourceFinalized)
}
...
func TestIntegration_LegacyGasStation_CrossChainTransfer_DestChain(t *testing.T) {
	...
	tx := types.LegacyGaslessTx{
		...
		SourceChainID:      testhelpers.SourceChainSelector,
		DestinationChainID: testhelpers.DestChainSelector,
