...
type SidecarDelegate struct {
	...
	ks     keystore.Eth
	db     *sqlx.DB
	lgsCfg config.LegacyGasStation
}
...
func (d *SidecarDelegate) ServicesForSpec(jb job.Job, qopts ...pg.QOpt) ([]job.ServiceCtx, error) {
	...
	if chain.Config().LegacyGasStation() != nil && chain.Config().LegacyGasStation().AuthConfig() != nil {
		mtlsCertificate = d.lgsCfg.AuthConfig().ClientCertificate
		mtlsKey = d.lgsCfg.AuthConfig().ClientKey
	}
