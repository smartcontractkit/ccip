...
func SignMetaTransfer(
	...
	metaMessage meta_erc20.MetaERC20MetaTransferMessage,
) (v uint8, r [32]byte, s [32]byte, err error) {
	nonce, err := metaToken.Nonces(nil, metaMessage.Owner)
	if err != nil {
		return 0, [32]byte{}, [32]byte{}, errors.Wrapf(err, "failed to get nonce of %s", metaMessage.Owner.Hex())
	}
...
`, typeHash, metaMessage.Owner, metaMessage.To, metaMessage.Amount, nonce, metaMessage.ChainId, metaMessage.GasLimit, metaMessage.Deadline,
	)
	if err != nil {
