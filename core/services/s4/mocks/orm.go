...
// Upsert provides a mock function with given fields: address, slotId, row, qopts
func (_m *ORM) Upsert(address common.Address, slotId uint, row *s4.Row, qopts ...pg.QOpt) error {
	_va := make([]interface{}, len(qopts))
	for _i := range qopts {
		_va[_i] = qopts[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, address, slotId, row)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 error
	if rf, ok := ret.Get(0).(func(common.Address, uint, *s4.Row, ...pg.QOpt) error); ok {
		r0 = rf(address, slotId, row, qopts...)
	} else {
		r0 = ret.Error(0)
	}
