...
contract BurnMintERC20Setup is BaseTest {
  ...
  uint256 internal s_maxSupply = 1e36;
...
contract BurnMintERC20_mint is BurnMintERC20Setup {
  ...
  function testSenderNotMinterReverts() public {
    // Minter role was granted in the constructor, so we must revert it for the test to error
...
contract BurnMintERC20_burn is BurnMintERC20Setup {
  ...
  function testSenderNotBurnerReverts() public {
    // Burner role was granted in the constructor, so we must revert it for the test to revert properly
...
contract BurnMintERC20_grantRole is BurnMintERC20Setup {
  ...
  function testGrantManySuccess() public {
    // These roles were granted in the constructor. It's easier to just revoke them and start fresh for the purpose
    // of the test
    s_burnMintERC20.revokeBurnRole(OWNER);
    s_burnMintERC20.revokeMintRole(OWNER);
