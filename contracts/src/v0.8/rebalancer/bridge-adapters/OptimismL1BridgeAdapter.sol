...
/// @notice L1 -> L2:
/// @notice L1 -> L2 deposits are done via the depositERC20To and depositETHTo functions on the L1StandardBridge.
/// @notice The amount of gas provided for the transaction must be buffered - the Optimism SDK recommends a 20% buffer.
/// @notice L2 -> L1:
/// @notice The Optimism Bridge implements 2-step withdrawals from L2 to L1. Once a withdrawal transaction is included
/// @notice in the L2 chain, it must be proven on L1 before it can be finalized. There is a buffer between the transaction
/// @notice being posted on L2 before it can be proven, and similarly, there is a buffer in the time it takes to prove
/// @notice the transaction before it can be finalized.
/// @notice See https://blog.oplabs.co/two-step-withdrawals/ for more details on this mechanism.
