...
contract EVM2EVMOffRamp_metadataHash is EVM2EVMOffRampSetup {
  function testMetadataHashSuccess() public {
    ...
    assertEq(h, keccak256(abi.encode(Internal.EVM_2_EVM_MESSAGE_HASH, SOURCE_CHAIN_ID, DEST_CHAIN_ID, ON_RAMP_ADDRESS)));
  }
}
...
contract EVM2EVMOffRamp_manuallyExecute is EVM2EVMOffRampSetup {
  ...
  function testReentrancyManualExecuteFAILS() public {
    ...
    assertEq(
      uint256(s_offRamp.getExecutionState(messages[0].sequenceNumber)), uint256(Internal.MessageExecutionState.FAILURE)
    );
