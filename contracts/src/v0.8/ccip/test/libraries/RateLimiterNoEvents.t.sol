...
contract RateLimiterNoEvents_setTokenBucketConfig is RateLimiterNoEventsSetup {
  function test_SetRateLimiterNoEventsConfig_Success() public {
    ...
    s_config = RateLimiterNoEvents.Config({
      isEnabled: true,
...
contract RateLimiterNoEvents_currentTokenBucketState is RateLimiterNoEventsSetup {
  function test_CurrentTokenBucketState_Success() public {
    ...
    s_config =
      RateLimiterNoEvents.Config({isEnabled: true, rate: uint128(bucket.rate * 2), capacity: bucket.capacity * 8});
...
contract RateLimiterNoEvents_consume is RateLimiterNoEventsSetup {
  ...
  function test_ConsumeUnlimited_Success() public {
    ...
    RateLimiterNoEvents.Config memory disableConfig =
      RateLimiterNoEvents.Config({isEnabled: false, rate: 0, capacity: 0});
  ...
  function test_ConsumingMoreThanUint128_Revert() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        RateLimiterNoEvents.AggregateValueMaxCapacityExceeded.selector, rateLimiter.capacity, request
      )
    );
  ...
  function test_AggregateValueRateLimitReached_Revert() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        RateLimiterNoEvents.AggregateValueRateLimitReached.selector,
        waitInSeconds,
  ...
  function test_TokenRateLimitReached_Revert() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        RateLimiterNoEvents.TokenRateLimitReached.selector,
        waitInSeconds,
        rateLimiter.capacity - requestTokens1,
        s_token
      )
  ...
  function test_RateLimitReachedOverConsecutiveBlocks_Revert() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(RateLimiterNoEvents.AggregateValueRateLimitReached.selector, 1, rateLimiter.rate)
    );
