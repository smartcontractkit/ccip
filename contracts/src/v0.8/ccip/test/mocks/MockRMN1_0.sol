...
contract MockRMN is IRMN, OwnerIsCreator {
  ...
  bytes private s_err;
  RMN1.VersionedConfig private s_versionedConfig;
  ...
  function ownerUnvoteToCurse(RMN1.UnvoteToCurseRecord[] memory) external {
    s_curse = false;
  }

  function ownerUnvoteToCurse(RMN1.UnvoteToCurseRecord[] memory, bytes16 subject) external {
  ...
  function getConfigDetails() external view returns (uint32 version, uint32 blockNumber, RMN1.Config memory config) {
