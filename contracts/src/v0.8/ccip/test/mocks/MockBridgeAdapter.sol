...
contract MockL1BridgeAdapter is IL1BridgeAdapter, ILiquidityContainer {
  ...
  function finalizeWithdraw(uint256 amount, bytes memory /* bridgeData */) external override {
    // Transfer the tokens to the caller
    // This is only valid for tests where the caller is the LiquidityManager contract
    // Note that this means that this adapter has to have the tokens in the first place
    // it should be set up during test setup.
...
contract MockL2BridgeAdapter is IBridgeAdapter {
  ...
  function finalizeWithdraw(uint256 /* amount */, bytes memory /* bridgeData */) external pure override {
    revert("todo");
  }
}
