...
//
//    bytes memory payload = abi.encodeWithSelector(s_facadeClient.send.selector, amount);
//
//    (bool success, bytes memory returnData) = address(s_facadeClient).call{gas: 1e18}(payload);
