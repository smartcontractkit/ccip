...
contract TokenPoolFactoryTests is TokenPoolFactorySetup {
  ...
  function test_createTokenPool_WithNoExistingRemoteContracts_predict_Success() public {
    ...
    TokenPoolFactory newTokenPoolFactory = new TokenPoolFactory(address(newTokenAdminRegistry), address(newRegistryModule), s_rmnProxy, address(s_destRouter));
    ...
    TokenPoolFactory.RemoteChainConfig memory remoteChainConfig = TokenPoolFactory.RemoteChainConfig(
      address(newTokenPoolFactory), address(s_destRouter), address(s_rmnProxy)
    ); 
    ...
    remoteTokenPools[0] = TokenPoolFactory.ExistingTokenPool(
      DEST_CHAIN_SELECTOR, abi.encode(s_tokenPoolFactory.EMPTY_PARAMETER_FLAG()), abi.encode(s_tokenPoolFactory.EMPTY_PARAMETER_FLAG()), s_tokenInitCode, RateLimiter.Config(false, 0, 0), RateLimiter.Config(false, 0, 0)
    );
    ...
    assertEq(abi.encode(predictedTokenAddress), TokenPool(poolAddress).getRemoteToken(DEST_CHAIN_SELECTOR), "Token Address should have been predicted");

    // Create the constructor params for the predicted pool
    // The predictedTokenAddress is NOT abi-encoded since the raw evm-address
    // is used in the constructor params
    bytes memory predictedPoolCreationParams = abi.encode(predictedTokenAddress, new address[](0), s_rmnProxy, address(s_destRouter));
    ...
    assertEq(abi.encode(predictedPoolAddress), TokenPool(poolAddress).getRemotePool(DEST_CHAIN_SELECTOR), "Pool Address should have been predicted");
    ...
    (address newTokenAddress, address newPoolAddress) = newTokenPoolFactory.deployTokenAndTokenPool(
      new TokenPoolFactory.ExistingTokenPool[](0), s_tokenInitCode, s_poolInitCode, abi.encode(s_tokenPoolFactory.EMPTY_PARAMETER_FLAG()), s_salt
    );

    assertEq(TokenPool(poolAddress).getRemotePool(DEST_CHAIN_SELECTOR), abi.encode(newPoolAddress), "New Pool Address should have been deployed correctly");

    assertEq(TokenPool(poolAddress).getRemoteToken(DEST_CHAIN_SELECTOR), abi.encode(newTokenAddress), "New Token Address should have been deployed correctly");
  }
