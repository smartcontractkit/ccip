...
contract RMNOffChainBlessor is IRMNOffChainBlessor, OwnerIsCreator, ITypeAndVersion {
  ...
  function verifySignatures(IAny2EVMMultiOffRamp.MerkleRoot calldata root) external view returns (bool) {
    ...
    for (uint256 i = 0; i < root.rmnSignatures.length; ++i) {
      ...
      address signer =
        ecrecover(keccak256(abi.encode(root.merkleRoot, root.interval.min, root.interval.min)), v + 27, r, s);
  ...
  /// @inheritdoc IRMNOffChainBlessor
