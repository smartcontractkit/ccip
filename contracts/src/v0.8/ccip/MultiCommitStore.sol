...
contract MultiCommitStore is IMultiCommitStore, ITypeAndVersion, OCR2Base {
  ...
  event SourceConfigUpdated(uint64 indexed sourceChainSelector, SourceConfig srcConfig);
  ...
  /// @dev Struct to hold the configs for a source chain, same as SourceConfig but with the sourceChainSelector
  /// so that an array of these can be passed in the constructor and the applySourceConfigUpdates function.
  struct SourceConfigArgs {
  ...
  // STATIC CONFIG
  string public constant override typeAndVersion = "MultiCommitStore 1.6.0";
  ...
  mapping(uint64 sourceChainSelector => SourceConfig srcConfig) private s_srcConfigs;
  ...
  /// @param srcConfigs An array of source chain specific configs
  ...
  constructor(StaticConfig memory staticConfig, SourceConfigArgs[] memory srcConfigs) OCR2Base(false) {
  ...
  function getSourceConfig(uint64 sourceChainSelector) external view returns (SourceConfig memory) {
    return s_srcConfigs[sourceChainSelector];
  }
  ...
  function getOnRamp(uint64 sourceChainSelector) external view returns (address) {
    return s_srcConfigs[sourceChainSelector].onRamp;
  }
  ...
  /// @notice Returns the source chain specific config.
  /// @param sourceChainSelector The source chain selector.
  /// @return the configuration.
  /// Note: should we revert on invalid sourceChainSelector as the OffRamp calls this function?
  function getSrcConfig(uint64 sourceChainSelector) external view returns (SourceConfig memory) {
    return s_srcConfigs[sourceChainSelector];
  }
