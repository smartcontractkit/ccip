...
import "../interfaces/Any2EVMMOOffRampInterface.sol";

/**
 * @notice Any2EVMMOOffRamp enables OCR networks to execute multiple messages
 * in an OffRamp in a single transaction.
 */
contract Any2EVMMOOffRamp is Any2EVMMOOffRampInterface, BaseOffRamp, TypeAndVersionInterface, OCR2Base {
  using Address for address;
  using SafeERC20 for IERC20;

  string public constant override typeAndVersion = "Any2EVMMOOffRamp 1.0.0";

  // The router through which all transactions will be executed
  Any2EVMMOOffRampRouterInterface public s_router;
   ...
   */
  function setRouter(Any2EVMMOOffRampRouterInterface router) external onlyOwner {
    s_router = router;
   ...
   * @param message The Any2EVMMOMessage message that will be executed
  ...
  /// @inheritdoc Any2EVMMOOffRampInterface
    ...
    whenHealthy
  {
    ...
    CCIP.Any2EVMMOMessage[] memory decodedMessages = new CCIP.Any2EVMMOMessage[](numMsgs);

    for (uint256 i = 0; i < numMsgs; ++i) {
      decodedMessages[i] = abi.decode(report.encodedMessages[i], (CCIP.Any2EVMMOMessage));
      // TODO: hasher
    ...
    for (uint256 i = 0; i < numMsgs; ++i) {
      ...
      CCIP.Any2EVMMOMessage memory message = decodedMessages[i];
  ...
  function _callReceiver(CCIP.Any2EVMMOMessage memory message) internal {
  ...
  function _trialExecute(CCIP.Any2EVMMOMessage memory message) internal returns (CCIP.MessageExecutionState) {
  ...
  function _isWellFormed(CCIP.Any2EVMMOMessage memory message) private view {
