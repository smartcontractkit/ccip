...
import {IRMNBase} from "./interfaces/IRMNBase.sol";
...
abstract contract RMNBase is IRMNBase, OwnerIsCreator, ITypeAndVersion {
  using EnumerableSet for EnumerableSet.AddressSet;

  // STATIC CONFIG
  uint256 internal constant MAX_NUM_VOTERS = 16;

  // MAGIC VALUES
  bytes28 internal constant NO_VOTES_CURSES_HASH = bytes28(0);
  ...
  VersionedConfig internal s_versionedConfig;
  ...
  mapping(address blessVoteAddr => BlesserRecord blesserRecord) internal s_blesserRecords;

  // Any tagged root with a commit store included in s_permaBlessedCommitStores will be considered automatically
  // blessed.
  EnumerableSet.AddressSet internal s_permaBlessedCommitStores;
  ...
  mapping(address curseVoteAddr => CurserRecord curserRecord) internal s_curserRecords;
  ...
  mapping(bytes16 subject => CurseVoteProgress curseVoteProgress) internal
  ...
  CurseHotVars internal s_curseHotVars;
  ...
  RecordedCurseRelatedOp[] internal s_recordedCurseRelatedOps;
