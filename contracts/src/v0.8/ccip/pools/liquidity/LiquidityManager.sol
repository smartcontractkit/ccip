...
contract LiquidityManager is ILiquidityManager, OCR3Base {
  ...
  function _receiveLiquidity(uint64 remoteChainSelector, uint256 amount, uint64 ocrSeqNum, bytes memory bridgeData) internal {
    // check if the remote chain is supported
    CrossChainLiquidityManager memory remoteLiqManager = s_crossChainLiquidityManager[remoteChainSelector];
    if (!remoteLiqManager.enabled) {
    ...
    remoteLiqManager.localBridge.finalizeWithdraw(amount, bridgeData);

    // inject liquidity into the liquidity container
    // approve and liquidity container should transferFrom
    bool success = i_localToken.approve(address(s_localLiquidityContainer), amount);
    require(success, "failed to approve for local liquidity container");
    s_localLiquidityContainer.provideLiquidity(amount);

    emit LiquidityTransferred(ocrSeqNum, remoteChainSelector, i_localChainSelector, address(this), amount);
  }
