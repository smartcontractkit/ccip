...
abstract contract MultiOCR3Abstract is ITypeAndVersion {
  ...
  /// @param donId DON ID for which the config was set
  ...
  /// @param donId DON ID to set the config for
  ...
  /// @param donId DON ID to return config details for
  /// @return blockNumber block at which this config was set
  /// @return configDigest domain-separation tag for current config (see _configDigestFromConfigData)
  function latestConfigDetails(uint32 donId) external view virtual returns (uint32 blockNumber, bytes32 configDigest);
  ...
  event Transmitted(uint32 indexed donId, bytes32 configDigest, uint64 sequenceNumber);
  ...
  /// @param donId DON ID to fetch config digest & sequence number for
  /// @return scanLogs indicates whether to rely on the configDigest and sequence number
  /// returned or whether to scan logs for the Transmitted event instead.
  /// @return configDigest
  /// @return sequenceNumber
  function latestConfigDigestAndEpoch(uint32 donId)
    external
    view
    virtual
  ...
  /// @param donId DON ID to transmit report for
